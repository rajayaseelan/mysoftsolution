    ...
    public class ScsResultMessage : ScsMessage
    {
        ...
        public override string ToString()
        {
            return string.IsNullOrEmpty(RepliedMessageId)
                       ? string.Format("ScsResultMessage [{0}]: ({1},{2})", MessageId, MessageValue.ServiceName, MessageValue.SubServiceName)
                       : string.Format("ScsResultMessage [{0}] Replied To [{1}]: ({2},{3})", MessageId, RepliedMessageId, MessageValue.ServiceName, MessageValue.SubServiceName);
        }
    }
}
