namespace MySoft.PlatformService.WinForm
{
    ...
    public class AutoCompleteTextbox : TextBox
    {
        ...
        public int SelectedIndex
        {
            ...
            {
                // musn't be null
                if (listBox.Items.Count != 0 && listBox.Items.Count > value)
        ...
        private void UpdateListBoxItems()
        {
            ...
            {
                //Get PointToScreen
                var p = this.PointToClient(this.ParentForm.Location);

                // get its width
                panel.Width = this.Width;
                // calculate the remeining height beneath the TextBox
                panel.Height = this.ParentForm.ClientSize.Height - Math.Abs(p.Y) - this.Height - 50;
                // and the Location to use
                panel.Location = new Point(Math.Abs(p.X) - 10, Math.Abs(p.Y) - this.Height + 10);
                ...
                if (!this.ParentForm.Controls.Contains(panel))
                {
                    ...
                    try { this.ParentForm.Controls.Add(panel); }
                    catch { }
                }
        ...
        private Point GetParentPoint(Control control)
        {
            if (control.Parent == null)
                return control.Location;
            else
                return GetParentPoint(control.Parent);
        }
