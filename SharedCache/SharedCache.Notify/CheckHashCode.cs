...
{
	/// <summary>
	/// Helps to identify which Hash Code function can be used.
	/// </summary>
	public partial class CheckHashCode : Form
	{
		// private thread to execute the logic and user able to see the form imidiatly
		private Thread worker = null;

		/// <summary>
		/// CTOR for CheckHashCode
		/// </summary>
		public CheckHashCode()
		{
			...
			this.Shown += new EventHandler(CheckHashCode_Shown);
		}
		...
		/// <param name="e"></param>
		void CheckHashCode_Shown(object sender, EventArgs e)
		{
		...
		/// A delegate method to invoke a method and prevent threading concurrent access
		/// </summary>
		/// <param name="simpleHashPossible"></param>
		private delegate void DelegateSetPossibleHashConfig(bool simpleHashPossible);
		...
		{
			this.ManageLink("http://www.sharedcache.com/cms/client_distributed_caching.aspx", false);
		}
	}
}
