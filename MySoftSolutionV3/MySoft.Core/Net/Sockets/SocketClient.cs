    ...
    public class SocketClient : SocketBase
    {
        ...
        private void ParseMessage(int TotalLength)
        {
            ...
            try
            {
                ...
                {
                    if ((TotalLength - ptr) <= 0)
                    {
                        SaveTheRestofTheStream(RawBuffer, ptr, TotalLength);
                        return;
                    }

                    int iLen = BitConverter.ToInt32(RawBuffer, 0);
        ...
        public bool Send(DataPacket o)
        {
            var buffer = BufferFormat.FormatFCA(o);
            return Send(buffer);
        }
        ...
        /// </summary>
        /// <param name="rawBuffer"> A Raw buffer of bytes to send </param>
        public bool Send(DataPacket o, out int length)
        {
            var buffer = BufferFormat.FormatFCA(o);
